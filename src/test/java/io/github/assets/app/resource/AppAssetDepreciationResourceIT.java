...
public class AppAssetDepreciationResourceIT {
    ...
    public void getAllAssetDepreciations() throws Exception {
        ...
        restAssetDepreciationMockMvc.perform(get("/api/app/asset-depreciations?sort=id,desc"))
                                    .andExpect(status().isOk())
    ...
    public void getAssetDepreciation() throws Exception {
        ...
        restAssetDepreciationMockMvc.perform(get("/api/app/asset-depreciations/{id}", assetDepreciation.getId()))
                                    .andExpect(status().isOk())
                                    .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    private void defaultAssetDepreciationShouldBeFound(String filter) throws Exception {
        restAssetDepreciationMockMvc.perform(get("/api/app/asset-depreciations?sort=id,desc&" + filter))
                                    .andExpect(status().isOk())
                                    .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
                                    .andExpect(jsonPath("$.[*].id").value(hasItem(assetDepreciation.getId().intValue())))
                                    .andExpect(jsonPath("$.[*].description").value(hasItem(DEFAULT_DESCRIPTION)))
        ...
        restAssetDepreciationMockMvc.perform(get("/api/app/asset-depreciations/count?sort=id,desc&" + filter))
                                    .andExpect(status().isOk())
                                    .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
